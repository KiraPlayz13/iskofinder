...
db.serialize(() => {
    db.run(`ALTER TABLE accounts ADD COLUMN location TEXT`, (err) => {
        if (err) {
            if (err.message.includes('duplicate column name')) {
                console.log('Column "location" already exists in accounts table.');
            } else {
                console.error('Failed to add location column:', err.message);
                process.exit(1);
            }
        } else {
            console.log('Successfully added "location" column to accounts table.');
        }
    });
});
